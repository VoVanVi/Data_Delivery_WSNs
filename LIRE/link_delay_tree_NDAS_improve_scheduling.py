...
def layering(node_list):
    """
    Layering based on link delay between nodes
    :param node_list:
    :return:
    """
    ...
    while len(remaining) > 0:
        ...
        for i in traversed:
            if node_list[i].layer == current_layer:
                for k in remaining:
                    if k in node_list[i].childrenIDs:
    ...
    for node_id in range(0, len(node_list)):
        node_list[node_id].current_prev_layer_neighbors = list(
            set(node_list[node_id].neighborIDs) - set(node_list[node_id].next_layer_neighbors))
        ...
        for wp in range(0, T):
            for slot in range(T - 1, -1, -1):
                for each_node in nodes_in_tree:
                    ...
                    temp = None
                    deg = T * 10
                    lst_nodes_smallest_link_delay_1 = []  # receiver_slot > sender_slot
                    lst_nodes_smallest_link_delay_2 = []  # receiver_slot <= sender_slot
                        ...
                        if receiver_slot == slot:
                            for each_neighbor in node_list[each_node].neighborIDs:
                                if each_neighbor not in nodes_in_tree:
                                    ...
                                    continue
                            if child != None and child not in nodes_in_tree:
                    ...
                    for receiver_slot in node_list[each_node].active_slot:
                        if receiver_slot == slot:
                            for each_neighbor in node_list[each_node].neighborIDs:
                                if each_neighbor not in nodes_in_tree:
                                    for sender_slot in node_list[each_neighbor].active_slot:
                                        if receiver_slot > sender_slot and receiver_slot - sender_slot < delay_1:
                                            delay_1 = receiver_slot - sender_slot
                                            child = each_neighbor
                                        if receiver_slot < sender_slot and receiver_slot + T - sender_slot < delay_2:
                                            delay_2 = receiver_slot + T - sender_slot
                                            child = each_neighbor
                    ...
                    """
                            for each_neighbor in node_list[each_node].neighborIDs:
                                if each_neighbor not in nodes_in_tree:
                                    for sender_slot in node_list[each_neighbor].active_slot:
                                        if receiver_slot > sender_slot and receiver_slot - sender_slot < delay:
                                            if receiver_slot - sender_slot == delay_1:
                                                lst_nodes_smallest_link_delay_1.append(each_neighbor)

                                        if receiver_slot < sender_slot and receiver_slot + T - sender_slot < delay:
                                            if receiver_slot + T - sender_slot == delay_2:
                                                lst_nodes_smallest_link_delay_2.append(each_neighbor)

                            #deg = 10
                            if len(lst_nodes_smallest_link_delay_1):
                                for nod in lst_nodes_smallest_link_delay_1:
                                    if len(node_list[nod].neighborIDs) < deg:
                                        deg = len(node_list[nod].neighborIDs)
                                        child = nod
                                        #print("sdfkbvdfbmd")
                                        #print(child)
                            else:
                                for nod in lst_nodes_smallest_link_delay_2:
                                    if len(node_list[nod].neighborIDs) < deg:
                                        deg = len(node_list[nod].neighborIDs)
                                        child = nod
                                        #print("aaaaaaaaaaaaa")
...
def NDAS_link_delay(node_list, T, m, comm_range, alpha):
    """
    :param node_list:
    :param T:
    :param m:
    ...
    """
    Sn = []
    node_list, max_layer, leaf_nodes, non_leaf_nodes = tree_construction_based_link_delay(node_list, T)

    links_from_leaf_nodes = find_parent_node_list(node_list, leaf_nodes)
    for i in range(1, len(links_from_leaf_nodes)):
    ...
    ts = links_selection(node_list, Sn, links_from_leaf_nodes, ts, T, m)
    """
    ...
    for i in range(max_layer, 0, -1):
        non_leaf_nodes_by_layer = []
        for each_node in range(0, len(node_list)):
            if node_list[each_node].layer == i:
                if each_node in non_leaf_nodes:
                    if node_list[each_node].channel is None and node_list[each_node].timeslot is None:
                        non_leaf_nodes_by_layer.append(each_node)
        links_from_non_leaf_nodes_by_layer = find_parent_node_list(node_list, non_leaf_nodes_by_layer)
        ts = links_selection(node_list, Sn, links_from_non_leaf_nodes_by_layer, ts, T, m)
...
def links_selection(node_list, Sn, list_of_links, ts, T, m):
    ...
    while list_of_links:
        ...
        # Apply algorithm 3
        te = link_schedule(node_list, Sc, ts, T, m)
        """
        ...
        """
        scheduled_links_before_Novel_schedule = []
        while len(scheduled_links) != len(scheduled_links_before_Novel_schedule):
            scheduled_links_before_Novel_schedule = copy.deepcopy(scheduled_links)
...
def novel_link_scheduling(node_list, scheduled_links, Sn, ts, te, T, m):
    for t in range(ts, te - 1):
        ...
        if mr != 0:
            ...
            Gs = defaultdict(list)
            for each_edge in Sn:
                if slot in node_list[each_edge[1]].active_slot and node_list[each_edge[1]].timeslot != t and node_list[
                    each_edge[1]].timeslot is None and check_scheduled_desc(node_list, each_edge[
                    0]) is True and check_timeslot_scheduled_children(node_list, each_edge[0], t) is True:
                    # if t > node_list[each_edge[0]].timeslot:
                    St.append(each_edge)
...
def link_schedule(node_list, candidate_links, ts, T, m):
    ...
    for slot, links in FACG.items():
        ...
        C = defaultdict(list, {k: [] for k in range(0, 5)})
        Cj = defaultdict(list)
        F = defaultdict(list)
        I = []
        # C[0].append(links[0])
    ...
    for each_link in candidate_links:
        for each_active_slot in node_list[each_link[1]].active_slot:
            if each_active_slot == time_slot:
                ...
                if considering_link[0] in node_list[each_link[1]].interfereIDs or considering_link[1] in node_list[
                    each_link[0]].interfereIDs:
                    degree += 1
    return degree
