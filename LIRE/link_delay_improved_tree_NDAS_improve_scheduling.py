...
def layering(node_list):
    ...
    for node_id in range(0, len(node_list)):
        node_list[node_id].current_prev_layer_neighbors = list(set(node_list[node_id].neighborIDs) - set(node_list[node_id].next_layer_neighbors))
...
def NDAS_link_delay(node_list, T, m, comm_range, alpha):
    ...
    for each_node in range(1, len(node_list)):
        print("channel and timeslot of each node: %u %u %u" % (each_node, node_list[each_node].channel, node_list[each_node].timeslot))
    ...
    """
    for each_node in range(1, len(node_list)):
        print("working period of each node: %u %u" % (each_node, node_list[each_node].wp))
    """
...
def novel_link_scheduling(node_list, scheduled_links, Sn, ts, te, T, m):
    for t in range(ts, te-1):
        ...
        if mr != 0:
            ...
            Gs = defaultdict(list)
            for each_edge in Sn:
                if slot in node_list[each_edge[1]].active_slot and node_list[each_edge[1]].timeslot != t and node_list[each_edge[1]].timeslot is None and check_scheduled_desc(node_list, each_edge[0]) is True and check_timeslot_scheduled_children(node_list, each_edge[0], t) is True:
...
def link_schedule(node_list, candidate_links, ts, T, m):
    ...
    for slot, links in FACG.items():
        ...
        """
        Add value j into 4th element of a link to be as channel of a transmitter node 
        """
