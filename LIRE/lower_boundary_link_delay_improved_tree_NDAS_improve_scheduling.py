...
def NDAS_link_delay(node_list, T, m, comm_range, alpha):
    ...
    while len(Sn) != 0:
        new_leaf_nodes = []
        ts, total_schedule_links = links_selection(node_list, Sn, links_from_leaf_nodes, ts, T, m)
        for each_link in total_schedule_links:
    ...
    for each_node in range(1, len(node_list)):
        print("channel and timeslot of each node: %u %u %u %u" % (
            each_node, node_list[each_node].channel, node_list[each_node].timeslot, node_list[each_node].delay))
...
def select_one_child(node_list, links_same_parent):
    ...
    """
    for each_link in links_same_parent:
        for each_as_rcv in node_list[each_link[1]].active_slot:
            for each_as_snd in node_list[each_link[0]].active_slot:
                if each_as_rcv - each_as_snd == node_list[each_link[0]].delay:
                    selected_link = each_link
    return selected_link
    """
...
def link_schedule(node_list, candidate_links, ts, T, m):
    ...
    for slot, links in FACG.items():
        ...
        C = defaultdict(list, {k: [] for k in range(0, 20)})
        Cj = defaultdict(list)
        F = defaultdict(list)
        I = []
