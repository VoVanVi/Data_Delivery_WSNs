...
def links_selection(node_list, Sn, list_of_links, ts, T, m):
    ...
    while list_of_links:
        ...
        """
        scheduled_links_after_Novel_schedule = copy.deepcopy(Sc_copy)
        scheduled_links_before_Novel_schedule = []
        while len(scheduled_links_after_Novel_schedule) != len(scheduled_links_before_Novel_schedule):
            scheduled_links_before_Novel_schedule = copy.deepcopy(scheduled_links_after_Novel_schedule)
            scheduled_links_after_Novel_schedule = novel_link_scheduling(node_list, scheduled_links_after_Novel_schedule, Sn, ts, te, T, m)

        #print("Sc_copy: ", Sc_copy)
        #scheduled_links_after_Novel_schedule = novel_link_scheduling(node_list, Sc_copy, Sn, ts, te, T, m)

        #total_scheduled_links = total_scheduled_links + scheduled_links_after_Novel_schedule
        ...
        """
        #print("scheduled_links_after_Novel_schedule: ", Sc_copy)
        #print("Sn: ", Sn)

        for each_link in scheduled_links_after_Novel_schedule:
            # if each_link in Sn:
            Sn.remove(each_link)
            if each_link in list_of_links:
                list_of_links.remove(each_link)
        ...
        if mr != 0:
            ...
            for each_edge in Sn:
                ...
                if slot in node_list[each_edge[1]].active_slot and node_list[each_edge[1]].timeslot is None and \
                        node_list[each_edge[1]].channel is None and check_rcv_not_yet_scheduled(node_list, each_edge[1], t) is True and node_list[each_edge[0]]. \
                        timeslot is None and node_list[each_edge[0]].channel is None and check_scheduled_desc(node_list, each_edge[0]) is True and \
...
def check_timeslot_scheduled_children(node_list, x, t):
    ...
    :param node_list:
    :param x: check if children of x are scheduled at timeslot t
...
def link_schedule(node_list, candidate_links, ts, T, m):
    ...
    for slot, links in FACG.items():
        ...
        C = defaultdict(list, {k: [] for k in range(0, 30)})
        Cj = defaultdict(list)
        F = defaultdict(list)
        I = []
        # C[0].append(links[0])
