...
def links_selection(node_list, Sn, list_of_links, ts, T, m):
    ...
    while list_of_links:
        ...
        for each_node in Sc:
            print(each_node)
            r = None
            for p in node_list[each_node].neighborIDs and node_list[each_node].timeslot = None:
                for active_slot in node_list[each_node].active_slot:
                    if active_slot = 
                if len(node_list[each_neighbor].neighborIDs) < len(node_list[parent_temp].neighborIDs):
                    parent_temp = node_list[each_neighbor]
            node_list[node_list[each_node].parentID].childrenIDs.remove(each_node)
        ...
        # Apply algorithm 3
        ts, unscheduled_nodes_Sc, unscheduled_edges_Sc = link_schedule(node_list, Sc, ts, T, m)
        ...
        for each_link in Sc_copy:
            if node_list[each_link[0]].channel != None:
                Sc_scheduled_nodes.append(each_link)
        """
        Apply algorithm 6 to schedule schedulable links in Sn from ts to (te-1)
        """
        scheduled_links = novel_link_scheduling(node_list, Sc_scheduled_nodes, Sn, ts, te, T, m)
        """
...
def link_schedule(node_list, candidate_links, ts, T, m):
    ...
    unscheduled_nodes_Sc = []
    unscheduled_edges_Sc = []
    ...
    for slot, links in FACG.items():
        ...
        for color_f, edges in F.items():
            for each_edge in edges:
                if node_list[each_edge[0]].timeslot is None and node_list[each_edge[0]].channel is None and color_f == 0:
                    node_list[each_edge[0]].timeslot = ts + color_f * T + slot
                    node_list[each_edge[0]].channel = each_edge[3]
                if node_list[each_edge[0]].timeslot is None and node_list[each_edge[0]].channel is None and color_f != 0:
                    unscheduled_nodes_Sc.append(each_edge[0])
                    unscheduled_edges_Sc.append((each_edge[0], each_edge[1]))
                    links.remove((each_edge[0], each_edge[1], each_edge[2]))
    ...
    return current_ts, unscheduled_nodes_Sc, unscheduled_edges_Sc
