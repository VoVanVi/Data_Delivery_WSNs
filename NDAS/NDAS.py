...
def NDAS(node_list, T, m, comm_range, alpha, black_nodes, blue_nodes, white_nodes, max_layer):
    ...
    for i in range(max_layer, 0, -1):
        ...
        #print("links_from_blue_nodes_by_layer", links_from_blue_nodes_by_layer)
        ts = links_selection(node_list, Sn, links_from_blue_nodes_by_layer, ts, T, m)
        #print("ts after blue node: ", ts)
        for each_node in range(0, len(node_list)):
            if node_list[each_node].layer == i:
        ...
        #print("links_from_black_nodes_by_layer", links_from_black_nodes_by_layer)
        ts = links_selection(node_list, Sn, links_from_black_nodes_by_layer, ts, T, m)
        #print("ts after black node: ", ts)
...
def NDAS_1(node_list, T, m, comm_range, alpha, max_layer, leaf_nodes, non_leaf_nodes):
...
def links_selection(node_list, Sn, list_of_links, ts, T, m):
    ...
    while list_of_links:
        ...
        #print("ts after link schedule", ts)
        """
