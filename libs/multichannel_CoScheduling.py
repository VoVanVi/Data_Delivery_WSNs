        ...
        for each_id_j in vertex_c:
            if node_list[each_id_j].ID != node_list[each_id_i].ID:
                ...
                if list(set(node_list[each_id_i].neighborIDs).intersection(set(node_list[each_id_j].childrenIDs))) != [] \
                        or list(
                    set(node_list[each_id_i].childrenIDs).intersection(set(node_list[each_id_j].neighborIDs))) != []:
...
def assign_working_period(node_list, scheduled_nodes_list, x):
    ...
    for each_id in node_list[x].childrenIDs:
        if node_list[each_id].channel != 0 and node_list[each_id].timeslot != 0 and node_list[
            each_id].wp not in wp_children_list:
...
def coscheduling(node_list, debug=False):
    ...
    while len(scheduled_nodes) + 1 != len(node_list):
        ...
        # I_prime is the set of nodes to avoid channel conflict with their children
        if node_list[u].rx_channel != 0:
            I_prime.append(node_list[u].rx_channel)
        for each_id in node_list[u].descIDs_prime:
            if debug:
                print("each DescIDs: ", each_id)
                print("receiving channel: ", node_list[each_id].rx_channel)
            if node_list[each_id].rx_channel != 0 and node_list[each_id].rx_channel not in I_prime:
                I_prime.append(node_list[each_id].rx_channel)
            else:
                for each_ID in node_list[each_id].childrenIDs:
                    if debug:
                        print("each children: ", each_ID)
                        print("receiving channel: ", node_list[each_ID].rx_channel)
                    if node_list[each_ID].rx_channel != 0 and node_list[each_ID].rx_channel not in I_prime:
                        I_prime.append(node_list[each_ID].rx_channel)
        ...
        for each_id in node_list[u].descIDs_prime:
            for each_node in scheduled_nodes:
                ...
                if each_node.ID not in vertex_c:
                    if each_node.ID in node_list[
                        each_id].neighborIDs and each_node.channel not in subtree_collision_list:
                        subtree_collision_list.append(each_node.channel)
        if debug:
            print("set(I)", set(I))
        ...
        if debug:
            print("The list I_prime to interference:", I_prime)
            print("The list subtree channel to avoid conflict", subtree_collision_list)
        ...
        if len(node_list[u].descIDs_prime) <= L:
            ...
            if node_list[u].wp > max(assigned_wp_list):
                ...
                """
                if len(I_prime) != 0:
                    node_list[u].rx_channel = min(list(set(Sch) - set(subtree_collision_list)))
                else:
                    node_list[u].rx_channel = min(list(set(Sch) - set(I) - set(subtree_collision_list)))
                """
                node_list[u].rx_channel = min(list(set(Sch) - set(I)))
            if debug:
